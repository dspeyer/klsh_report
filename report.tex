...
Once we have the cr-nn problem solved, we can use that to solve the
Aproximate Nearest Neighbor problem: finding a point in our database
...
\begin{equation}
  h(x) = \sign \ \langle \Phi(x), \Sigma_S^{-1/2} z\rangle = \sign\left(\sum_{i=1}^t w_i \cdot K(x_i, x)\right),\
\end{equation}
