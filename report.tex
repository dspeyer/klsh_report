...
\end{proposition}
It is particularly amazing because $\mathcal{X}$ was just a set; the only thing we needed required was a positive-definite kernel $K$ to obtain $\mathcal{H}$. And in fact:
\begin{fact}
...
This is a powerful result because we may now view $\mathcal{X}$ not as an abstract set, but as vectors within a Hilbert space, which carries with it both algebraic and geometric properties which $\mathcal{X}$ now inherits.

Despite this, the way we've presented the embedding of $\mathcal{X}$ into $\mathcal{H}$ is somewhat unnatural. While it makes sense to consider the collection of functions $k_x$, the identification of $x$ with $k_x$ may seem \emph{ad hoc}. But if we allow ourselves to borrow some powerful facts from functional analysis, we may obtain a deeper and more natural understanding of RKHS's. Indeed, hopefully we can clarify why we might call $\Phi$ a \emph{feature map}.

\subsection{Feature Map}
Let us once again consider an abstract set $\mathcal{X}$, and a collection of functions over $\mathcal{X}$, say $\mathcal{H} \subset \bbR^\mathcal{X}$. We may imagine each $f \in \mathcal{H}$ as an observable or a feature, where the value $f(x)$ gives the value after \emph{measuring} the object $x$ with feature $f$.

As a result, every $x \in \mathcal{X}$ is associated to the collection of measurements,
...
Here, it makes sense to call the mapping in Equation~\ref{dual} the \emph{feature map}, for it maps the cat to its features, specifically its colors at specific locations. In general, given any function $f : \mathcal{X} \to \bbR$, we may dually view points of $x$ as functionals, $\mathrm{ev}_x : \mathcal{H} \to \bbR$, where
\[\mathrm{ev}_x(f) := f(x).\]
In functional analysis, we say that $\mathrm{ev}_x$ is the \emph{evaluation at $x$}, and we view it as an element of the \emph{dual space} $\bbR^\mathcal{H}$ of $\mathcal{H}$, which we denote by $\mathcal{H}^*$.\footnote{Many cases, $\mathcal{H}^*$ will denote the \emph{continuous dual} of $\mathcal{H}$, and not the \emph{algebraic dual}. Indeed, we want to consider the continuous dual here, the collection of continuous functionals on $\mathcal{H}$.} To recapitulate, $\Phi(x) = \mathrm{ev}_x$.

Return back to kernels as measures of similarity,
