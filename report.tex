...
\subsection{LSH on the Sphere} %dspeyer
...
Let's examine the first problem and understand why an infinite-dimensional Hilbert space may not
have a canonical Gaussian distribution---this will actually give us some more intuition on how
$\Phi$ can embed $\mathcal{X}$ into $\mathcal{H}$. Following this, a natural solution to both
problems will follow.

\subsection{Infinite-Dimensional Normal Distributions} % geelon

\begin{proposition}[{\cite[Example 2.2]{L2012}}] Let $\mathcal{H}$ be a separable Hilbert space.
...
In particular, this implies that no analog of the normal distribution $\mathcal{N}(0,I)$ exists in $\cH$ as the trace of $I$ is infinite. Although we won't prove this,\footnote{Additionally, the reader might notice that we have not actually defined what a Gaussian measure on $\cH$ is; we refer the reader to \cite{L2012} for details.} we can give intuition: the normal distribution in $\bbR^n$ is the product of $n$ independent normal Gaussians along $n$ orthogonal directions. As the number of dimensions go to infinity, we would find that the probability distribution would converges to 0 on $\mathcal{H}$.

However, as the above proposition suggests, this is not to say that no Gaussian measures exist in $\cH$. Just that `most' of the variance of a Gaussian distribution will be concentrated within a finite number of directions.

So heuristically, if we begin to think about how $\Phi(X)$ may be distributed within $\mathcal{H}$,
